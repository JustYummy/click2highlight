...
// @copyright       2018 AC
// @description     双击选中高亮
// @note            2018.09.22-V2.6 新增颜色，新增多次选择功能
// @note            2018.09.01-V2.5 去除复制功能
...
//document.addEventListener('keyup', ClearAll, false);
...
//    var textSet = ""
    ...
    if (textSet!="") {
        selectedText = selectedText.replace(/[^\u4E00-\u9FA5|0-9|a-z|A-Z_]+/g, "");
        if(selectedText!=""){
            selectedText = textSet + "_" + selectedText;
            textSet = selectedText;
            countNum ++;
        }
    } else {
        textSet = selectedText.replace(/[^\u4E00-\u9FA5|0-9|a-z|A-Z_]+/g, "");
        if (textSet!=""){
            countNum ++;
        }
    }
   // coutnum++;
    ...
    if (selectedText && getBLen(selectedText) >= 3) {
        myConsoleLog("DoHighLight " + selectedText)
        if (s_keyup){
...
//    GM_setClipboard(selectedText);
    ...
    myConsoleLog("双击的:" +keySets.keywords);
    doHighLightAll_CSS();
    doHighLightAll_Text();
}
function getSelectedText(target) {
    ...
    myConsoleLog("getSelectedText:" +selectedText);
    return selectedText;
}
